...
$adminControllerIds = [
    ...
    'contactos', 'folios', 'categorias', 'mascotas',
];
...
$adminModuleNames = [
    ...
    'mascotas'       => 'Mascota Tigre',
];

// Mascota Tigre: datos administrables (se degrada en silencio si no está migrada)
$mascotasData = [];
try {
    $mascotasData = array_map(
        fn(\app\models\Mascotas $m) => $m->toApiArray(),
        \app\models\Mascotas::find()->where(['activo' => 1])->all()
    );
} catch (\Throwable) {
    // Tabla aún no migrada — la mascota no se muestra.
}
$ogMascota = $mascotasData[0]['imagen'] ?? null;
?>
<?php $this->beginPage() ?>
<!DOCTYPE html>
<html lang="es">
...
<?php if (!empty($mascotasData)): ?>
<!-- Mascota Tigre (administrable) -->
<div id="tigreOverlay" aria-hidden="true">
  <div id="tigreUnit">
    <button type="button" id="tigreClose" aria-label="Cerrar mascota">&times;</button>
    <div id="tigreBubble"><p id="tigreBubbleText"></p></div>
    <div id="tigreImgWrap">
      <img id="tigreImg" src="" alt="Mascota ARGA Group">
      <a id="tigreCta" href="#" target="_blank" rel="noopener" aria-label="Escríbenos por WhatsApp">
...
<script type="application/json" id="mascotaData"><?= json_encode($mascotasData, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?></script>
<?php endif; ?>
