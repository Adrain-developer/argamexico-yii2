<!DOCTYPE html>
<html lang="es">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Arga Group</title>
  <link rel="preconnect" href="https://fonts.googleapis.com/" />
  <link rel="preconnect" href="https://fonts.gstatic.com/" crossorigin />
  <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700;800;900&amp;display=swap" rel="stylesheet" />
  <link rel="stylesheet" href="/demo/styles.css" />
</head>
<body>

  <!-- SVG gradient defs globales -->
  <svg width="0" height="0" style="position:absolute" aria-hidden="true">
    <defs>
      <linearGradient id="tealGrad" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#4A82D8"/>
        <stop offset="100%" stop-color="#2E6AC9"/>
      </linearGradient>
      <linearGradient id="greenGrad" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#E8304A"/>
        <stop offset="100%" stop-color="#C41230"/>
      </linearGradient>
    </defs>
  </svg>

  <!-- ===== HEADER ===== -->
  <header class="site-header" id="siteHeader">
    <div class="header-inner">
      <a href="#inicio" class="logo" aria-label="ARGA Group inicio">
        <div class="logo-wrap">
          <img src="/demo/img/argaBlanco.png" alt="ARGA Group México" class="logo-img logo-blanco" />
          <img src="/demo/img/argaColor.png"  alt="ARGA Group México" class="logo-img logo-color" />
        </div>
      </a>

      <nav class="main-nav" id="mainNav">
        <ul>
          <li><a href="#inicio" class="nav-link active">Inicio</a></li>
          <li class="has-dropdown">
            <a href="#quienes" class="nav-link">Nosotros <span class="arrow">▾</span></a>
            <ul class="dropdown">
              <li><a href="#quienes">Quiénes Somos</a></li>
              <li><a href="#mision">Misión, Visión y Valores</a></li>
              <li><a href="#equipo">Equipo de Trabajo</a></li>
              <li><a href="#politicas">Políticas</a></li>
            </ul>
          </li>
          <li class="has-dropdown">
            <a href="#servicios" class="nav-link">ARGA Fire <span class="arrow">▾</span></a>
            <ul class="dropdown">
              <li><a href="#servicios">Determinación de Riesgo de Incendio</a></li>
              <li><a href="#servicios">Capacitación en Campo de Prácticas</a></li>
              <li><a href="#servicios">Sistemas de Protección contra Incendio</a></li>
            </ul>
          </li>
          <li class="has-dropdown">
            <a href="#servicios" class="nav-link">ARGA Consultores <span class="arrow">▾</span></a>
            <ul class="dropdown">
              <li><a href="#servicios">Seguridad y Salud en el Trabajo</a></li>
              <li><a href="#servicios">Higiene Industrial</a></li>
              <li><a href="#servicios">Medio Ambiente</a></li>
              <li><a href="#servicios">Sustancias Químicas Peligrosas</a></li>
              <li><a href="#servicios">Requerimientos Legales y Normativos</a></li>
              <li><a href="#servicios">Factores de Riesgos Psicosociales</a></li>
              <li><a href="#servicios">Ergonomía</a></li>
              <li><a href="#servicios">Equipo de Protección Personal</a></li>
            </ul>
          </li>
          <li class="has-dropdown">
            <a href="#servicios" class="nav-link">ARGA Labs <span class="arrow">▾</span></a>
            <ul class="dropdown">
              <li><a href="#servicios">Laboratorio de Pruebas Acreditado</a></li>
              <li><a href="#servicios">Muestreo de Agentes Físicos</a></li>
              <li><a href="#servicios">Análisis de Agentes Químicos</a></li>
              <li><a href="#servicios">Unidad de Auditoría e Inspección</a></li>
            </ul>
          </li>
          <li class="has-dropdown">
            <a href="#servicios" class="nav-link">ARGA Training <span class="arrow">▾</span></a>
            <ul class="dropdown">
              <li><a href="#servicios">Capacitación Presencial</a></li>
              <li><a href="#servicios">Capacitación en Línea</a></li>
              <li><a href="#servicios">Cursos Especializados</a></li>
              <li><a href="#servicios">Diplomados</a></li>
            </ul>
          </li>
          <li><a href="#contacto" class="nav-link">Contacto</a></li>
        </ul>
      </nav>

      <a href="#" class="btn-portal">Soy cliente</a>
      <button class="hamburger" id="hamburger" aria-label="Menú">
        <span></span><span></span><span></span>
      </button>
    </div>
  </header>

  <!-- ===== HERO SLIDER ===== -->
  <section class="hero" id="inicio">
    <div class="slides-container" id="slidesContainer">

      <!-- Slide 1 — Higiene Industrial (imagen derecha) -->
      <div class="slide slide-1 active">
        <div class="slide-img-wrap slide-img-right">
          <img src="/demo/img/1.png" alt="Higiene Industrial" class="slide-img">
          <div class="slide-img-fade slide-img-fade-right"></div>
        </div>
        <div class="slide-content">
          <h1>Higiene Industrial</h1>
          <p>Reconocimiento y evaluación de agentes físicos: ruido, iluminación, vibraciones, temperaturas y electricidad estática. Acreditados por la EMA.</p>
          <a href="#servicios" class="btn-cta">Ver Más</a>
        </div>
      </div>

      <!-- Slide 2 — Estudios de Seguridad / Consultoría (imagen izquierda) -->
      <div class="slide slide-2">
        <div class="slide-img-wrap slide-img-left">
          <img src="/demo/img/2.png" alt="Estudios de Seguridad" class="slide-img">
          <div class="slide-img-fade slide-img-fade-left"></div>
        </div>
        <div class="slide-content">
          <h1>Estudios de Seguridad<br>y Consultoría</h1>
          <p>Cumplimiento normativo STPS en instalaciones, maquinaria, sustancias químicas, ergonomía y factores de riesgo psicosocial.</p>
          <a href="#servicios" class="btn-cta">Ver Más</a>
        </div>
      </div>

      <!-- Slide 3 — Recipientes a Presión (imagen derecha) -->
      <div class="slide slide-3">
        <div class="slide-img-wrap slide-img-right">
          <img src="/demo/img/3.png" alt="Recipientes a Presión" class="slide-img">
          <div class="slide-img-fade slide-img-fade-right"></div>
        </div>
        <div class="slide-content">
          <h1>Recipientes a Presión<br>y Dictaminaciones</h1>
          <p>Dictaminación de calderas, recipientes criogénicos y tuberías. Ensayos no destructivos: ultrasonido, partículas magnéticas y líquidos penetrantes.</p>
          <a href="#servicios" class="btn-cta">Ver Más</a>
        </div>
      </div>

      <!-- Slide 4 — Sistema Contra Incendios (imagen izquierda) -->
      <div class="slide slide-4">
        <div class="slide-img-wrap slide-img-left">
          <img src="/demo/img/4.png" alt="Sistemas Contra Incendios" class="slide-img">
          <div class="slide-img-fade slide-img-fade-left"></div>
        </div>
        <div class="slide-content">
          <h1>Sistemas Contra<br>Incendios</h1>
          <p>Diseño, instalación y mantenimiento de sistemas de extinción, alarma, detección, hidrantes y rociadores. +30 años de experiencia en México y LATAM.</p>
          <a href="#servicios" class="btn-cta">Ver Más</a>
        </div>
      </div>

      <!-- Slide 5 — Capacitación (imagen derecha) -->
      <div class="slide slide-5">
        <div class="slide-img-wrap slide-img-right">
          <img src="/demo/img/5.png" alt="Capacitación" class="slide-img">
          <div class="slide-img-fade slide-img-fade-right"></div>
        </div>
        <div class="slide-content">
          <h1>Capacitación</h1>
          <p>Cursos registrados ante STPS y PROFEPA: LOTO, espacios confinados, trabajo en altura, montacargas, brigadas contra incendio y más.</p>
          <a href="#servicios" class="btn-cta">Ver Más</a>
        </div>
      </div>

    </div>

    <!-- Slider controls -->
    <button class="slide-arrow prev" id="prevSlide" aria-label="Anterior">&#8249;</button>
    <button class="slide-arrow next" id="nextSlide" aria-label="Siguiente">&#8250;</button>

    <!-- Dots -->
    <div class="slide-dots" id="slideDots">
      <button class="dot active" data-index="0"></button>
      <button class="dot" data-index="1"></button>
      <button class="dot" data-index="2"></button>
      <button class="dot" data-index="3"></button>
      <button class="dot" data-index="4"></button>
    </div>

  </section>

  <!-- ===== QUIÉNES SOMOS + UNIDADES DE NEGOCIO ===== -->
  <section class="section unidades-section reveal" id="quienes">
    <div class="container">
      <div class="unidades-header">
        <h2 class="section-title gradient-text">QUIÉNES SOMOS</h2>
        <p class="section-subtitle">
          Más de 30 años colaborando en el desarrollo de empresas más<br>
          <strong>Eficientes, Seguras y Rentables</strong>
        </p>
      </div>
      <div class="unidades-inner">

        <div class="unidades-text">
          <h2 class="unidades-title">
            <span class="gradient-text">DIVISIONES</span><br>
            <span class="gradient-text-alt">DE NEGOCIO</span>
          </h2>
          <p>Respaldada por profesionales con más de 17 años de experiencia en el ámbito de la seguridad industrial, salud ocupacional, protección ambiental, seguridad patrimonial, emergencia y desarrollo humano. Dicha experiencia ha quedado demostrada en empresas del giro de la construcción, petroquímica, alimentaría, industrial, comercial y de servicios. Donde el principal objetivo ha sido brindar un servicio de calidad, garantizando la integridad física de los colaboradores, la productividad, la continuidad del negocio, las afectaciones ambientales y la reducción de pérdidas por sanciones administrativas.</p>
        </div>

        <div class="unidades-grid-wrap">
          <!-- Watermark SVG icons de fondo -->
          <svg class="unidades-watermark-svg" aria-hidden="true" viewBox="0 0 600 480" fill="none" xmlns="http://www.w3.org/2000/svg">
            <!-- Casco seguridad -->
            <g transform="translate(20,20)" opacity=".13" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M5 28 Q5 12 30 12 Q55 12 55 28 Z" fill="none"/>
              <line x1="0" y1="28" x2="60" y2="28"/>
              <path d="M15 28 v5 Q15 38 30 38 Q45 38 45 33 v-5"/>
            </g>
            <!-- Llave inglesa -->
            <g transform="translate(95,10)" opacity=".12" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M10 40 L35 15" stroke-linecap="round"/>
              <circle cx="10" cy="45" r="8" fill="none"/>
              <circle cx="37" cy="13" r="6" fill="none"/>
            </g>
            <!-- Microscopio -->
            <g transform="translate(180,15)" opacity=".11" stroke="#3a5a8c" stroke-width="1.5">
              <line x1="25" y1="8" x2="25" y2="30"/>
              <ellipse cx="25" cy="6" rx="8" ry="4" fill="none"/>
              <path d="M16 30 Q10 40 10 46 Q10 52 25 52 Q40 52 40 46 Q40 40 34 30 Z" fill="none"/>
              <line x1="10" y1="46" x2="40" y2="46"/>
            </g>
            <!-- Extintor -->
            <g transform="translate(280,8)" opacity=".12" stroke="#3a5a8c" stroke-width="1.5">
              <rect x="10" y="18" width="20" height="34" rx="10" fill="none"/>
              <line x1="20" y1="8" x2="20" y2="18"/>
              <path d="M20 8 Q30 4 36 8"/>
              <path d="M30 16 Q40 14 42 22"/>
            </g>
            <!-- Engranaje -->
            <g transform="translate(390,5)" opacity=".10" stroke="#3a5a8c" stroke-width="1.5">
              <circle cx="25" cy="25" r="10" fill="none"/>
              <circle cx="25" cy="25" r="5" fill="none"/>
              <line x1="25" y1="8" x2="25" y2="14"/><line x1="25" y1="36" x2="25" y2="42"/>
              <line x1="8" y1="25" x2="14" y2="25"/><line x1="36" y1="25" x2="42" y2="25"/>
              <line x1="13" y1="13" x2="17" y2="17"/><line x1="33" y1="33" x2="37" y2="37"/>
              <line x1="37" y1="13" x2="33" y2="17"/><line x1="17" y1="33" x2="13" y2="37"/>
            </g>
            <!-- Imágenes diagnóstico / QR-like -->
            <g transform="translate(510,12)" opacity=".10" stroke="#3a5a8c" stroke-width="1.5">
              <rect x="2" y="2" width="14" height="14" fill="none"/>
              <rect x="5" y="5" width="8" height="8" fill="none"/>
              <rect x="24" y="2" width="14" height="14" fill="none"/>
              <rect x="27" y="5" width="8" height="8" fill="none"/>
              <rect x="2" y="24" width="14" height="14" fill="none"/>
              <rect x="5" y="27" width="8" height="8" fill="none"/>
              <line x1="24" y1="24" x2="38" y2="38"/>
              <line x1="38" y1="24" x2="24" y2="38"/>
            </g>
            <!-- Figura persona/trabajador -->
            <g transform="translate(30,110)" opacity=".10" stroke="#3a5a8c" stroke-width="1.5">
              <circle cx="20" cy="8" r="7" fill="none"/>
              <path d="M6 35 Q6 22 20 22 Q34 22 34 35"/>
              <line x1="20" y1="22" x2="20" y2="45"/>
              <line x1="8" y1="30" x2="32" y2="30"/>
              <line x1="20" y1="45" x2="10" y2="58"/>
              <line x1="20" y1="45" x2="30" y2="58"/>
            </g>
            <!-- Matraz químico -->
            <g transform="translate(120,100)" opacity=".11" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M18 6 v18 L6 44 Q2 52 12 52 Q22 58 28 52 Q38 52 34 44 L22 24 v-18 Z" fill="none"/>
              <line x1="15" y1="6" x2="25" y2="6"/>
              <circle cx="14" cy="42" r="3" fill="none"/>
              <circle cx="26" cy="48" r="2" fill="none"/>
            </g>
            <!-- Termómetro -->
            <g transform="translate(220,95)" opacity=".10" stroke="#3a5a8c" stroke-width="1.5">
              <line x1="20" y1="10" x2="20" y2="38"/>
              <circle cx="20" cy="44" r="7" fill="none"/>
              <rect x="16" y="10" width="8" height="30" rx="4" fill="none"/>
              <line x1="24" y1="20" x2="30" y2="20"/>
              <line x1="24" y1="28" x2="30" y2="28"/>
            </g>
            <!-- Fuego / llama -->
            <g transform="translate(320,95)" opacity=".10" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M20 50 Q6 40 10 24 Q14 14 20 8 Q20 20 26 16 Q34 28 28 36 Q36 30 34 18 Q44 30 38 44 Q34 52 20 50 Z" fill="none"/>
            </g>
            <!-- Portapapeles / checklist -->
            <g transform="translate(420,100)" opacity=".11" stroke="#3a5a8c" stroke-width="1.5">
              <rect x="6" y="10" width="32" height="40" rx="2" fill="none"/>
              <rect x="14" y="4" width="16" height="10" rx="2" fill="none"/>
              <line x1="12" y1="22" x2="32" y2="22"/>
              <line x1="12" y1="30" x2="32" y2="30"/>
              <line x1="12" y1="38" x2="24" y2="38"/>
              <path d="M26 36 l3 3 6-6" stroke-linecap="round"/>
            </g>
            <!-- Señal advertencia -->
            <g transform="translate(530,105)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M22 4 L42 42 H2 Z" fill="none"/>
              <line x1="22" y1="16" x2="22" y2="28"/>
              <circle cx="22" cy="35" r="2" fill="#3a5a8c" opacity=".3"/>
            </g>
            <!-- Red/nodos conexión -->
            <g transform="translate(50,220)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <circle cx="8" cy="8" r="5" fill="none"/>
              <circle cx="38" cy="8" r="5" fill="none"/>
              <circle cx="23" cy="36" r="5" fill="none"/>
              <line x1="13" y1="8" x2="33" y2="8"/>
              <line x1="11" y1="12" x2="20" y2="31"/>
              <line x1="35" y1="12" x2="26" y2="31"/>
            </g>
            <!-- Ojo / inspección -->
            <g transform="translate(160,215)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M4 22 Q22 6 40 22 Q22 38 4 22 Z" fill="none"/>
              <circle cx="22" cy="22" r="7" fill="none"/>
              <circle cx="22" cy="22" r="3" fill="none"/>
            </g>
            <!-- Foco / idea -->
            <g transform="translate(255,210)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <circle cx="20" cy="18" r="12" fill="none"/>
              <path d="M13 28 Q13 36 20 36 Q27 36 27 28"/>
              <line x1="16" y1="36" x2="24" y2="36"/>
              <line x1="20" y1="6" x2="20" y2="2"/>
              <line x1="4" y1="18" x2="0" y2="18"/>
              <line x1="36" y1="18" x2="40" y2="18"/>
              <line x1="7" y1="7" x2="4" y2="4"/>
              <line x1="33" y1="7" x2="36" y2="4"/>
            </g>
            <!-- Mano protección -->
            <g transform="translate(360,210)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M20 40 v-20 M14 40 v-16 M8 40 v-12 M26 40 v-18 M32 40 v-14" stroke-linecap="round"/>
              <path d="M8 28 Q8 20 20 20 Q32 20 32 26 v14 Q32 46 20 46 Q8 46 8 40 Z" fill="none"/>
            </g>
            <!-- Gota agua / medio ambiente -->
            <g transform="translate(468,215)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M20 6 Q34 22 34 32 Q34 46 20 46 Q6 46 6 32 Q6 22 20 6 Z" fill="none"/>
            </g>
            <!-- Candado -->
            <g transform="translate(555,218)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <rect x="6" y="22" width="28" height="22" rx="3" fill="none"/>
              <path d="M10 22 v-8 Q10 6 20 6 Q30 6 30 14 v8"/>
              <circle cx="20" cy="32" r="3" fill="none"/>
              <line x1="20" y1="35" x2="20" y2="38"/>
            </g>
            <!-- Cámara CCTV -->
            <g transform="translate(80,340)" opacity=".09" stroke="#3a5a8c" stroke-width="1.5">
              <rect x="4" y="14" width="28" height="18" rx="2" fill="none"/>
              <polygon points="32,18 44,12 44,30 32,26" fill="none"/>
              <circle cx="16" cy="23" r="5" fill="none"/>
            </g>
            <!-- Nube datos -->
            <g transform="translate(185,335)" opacity=".08" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M10 36 Q4 36 4 28 Q4 20 12 20 Q12 10 22 10 Q32 10 34 18 Q42 18 42 26 Q42 36 34 36 Z" fill="none"/>
              <line x1="22" y1="36" x2="22" y2="46"/>
              <line x1="16" y1="42" x2="28" y2="42"/>
            </g>
            <!-- Libro norma -->
            <g transform="translate(300,338)" opacity=".08" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M6 8 Q6 4 14 4 Q18 4 20 8 Q22 4 26 4 Q34 4 34 8 v36 Q34 48 26 44 Q22 48 20 44 Q18 48 14 44 Q6 48 6 44 Z" fill="none"/>
              <line x1="20" y1="8" x2="20" y2="44"/>
            </g>
            <!-- Escudo protección -->
            <g transform="translate(415,335)" opacity=".08" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M20 4 L36 10 v18 Q36 36 20 46 Q4 36 4 28 v-18 Z" fill="none"/>
              <path d="M12 22 l6 6 12-12" stroke-linecap="round" stroke-linejoin="round"/>
            </g>
            <!-- Radiación / átomo -->
            <g transform="translate(530,340)" opacity=".08" stroke="#3a5a8c" stroke-width="1.5">
              <circle cx="22" cy="22" r="4" fill="none"/>
              <ellipse cx="22" cy="22" rx="18" ry="8" fill="none"/>
              <ellipse cx="22" cy="22" rx="18" ry="8" fill="none" transform="rotate(60 22 22)"/>
              <ellipse cx="22" cy="22" rx="18" ry="8" fill="none" transform="rotate(120 22 22)"/>
            </g>
            <!-- Herramienta / destornillador -->
            <g transform="translate(140,430)" opacity=".08" stroke="#3a5a8c" stroke-width="1.5">
              <line x1="10" y1="10" x2="36" y2="36" stroke-linecap="round"/>
              <path d="M36 30 Q44 22 42 14 Q34 12 26 20 Z" fill="none"/>
              <line x1="6" y1="14" x2="14" y2="6"/>
            </g>
            <!-- Monitor / pantalla -->
            <g transform="translate(280,428)" opacity=".08" stroke="#3a5a8c" stroke-width="1.5">
              <rect x="4" y="4" width="36" height="26" rx="2" fill="none"/>
              <line x1="20" y1="30" x2="20" y2="40"/>
              <line x1="12" y1="40" x2="28" y2="40"/>
              <line x1="10" y1="18" x2="34" y2="18"/>
            </g>
            <!-- Corazón médico -->
            <g transform="translate(420,430)" opacity=".07" stroke="#3a5a8c" stroke-width="1.5">
              <path d="M22 38 Q4 26 4 16 Q4 6 14 6 Q18 6 22 12 Q26 6 30 6 Q40 6 40 16 Q40 26 22 38 Z" fill="none"/>
              <line x1="14" y1="18" x2="30" y2="18"/>
              <line x1="22" y1="10" x2="22" y2="26"/>
            </g>
          </svg>

          <div class="unidades-grid">

            <!-- Capacitación -->
            <div class="unidad-item">
              <svg class="unidad-shield teal" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- libro abierto -->
                <path d="M24 35 Q40 30 40 30 Q40 30 56 35 L56 58 Q40 53 40 53 Q40 53 24 58 Z" stroke="white" stroke-width="2" fill="none" stroke-linejoin="round"/>
                <line x1="40" y1="30" x2="40" y2="53" stroke="white" stroke-width="2"/>
                <path d="M28 40 Q34 38 38 39M28 45 Q34 43 38 44M28 50 Q34 48 38 49" stroke="white" stroke-width="1.2" stroke-linecap="round" opacity=".8"/>
                <path d="M42 39 Q46 38 52 40M42 44 Q46 43 52 45M42 49 Q46 48 52 50" stroke="white" stroke-width="1.2" stroke-linecap="round" opacity=".8"/>
              </svg>
              <p>Capacitación</p>
            </div>

            <!-- Salud Ocupacional -->
            <div class="unidad-item">
              <svg class="unidad-shield green" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- caduceo / médico -->
                <circle cx="40" cy="34" r="10" stroke="white" stroke-width="2" fill="none"/>
                <path d="M36 30 h8 M40 26 v8" stroke="white" stroke-width="2.5" stroke-linecap="round"/>
                <path d="M22 58 Q22 46 40 46 Q58 46 58 58" stroke="white" stroke-width="2" fill="none" stroke-linecap="round"/>
              </svg>
              <p>Salud Ocupacional</p>
            </div>

            <!-- Auditoría e Inspección -->
            <div class="unidad-item">
              <svg class="unidad-shield teal" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- candado auditoría -->
                <rect x="26" y="42" width="28" height="20" rx="3" stroke="white" stroke-width="2" fill="none"/>
                <path d="M30 42 V35 Q30 26 40 26 Q50 26 50 35 V42" stroke="white" stroke-width="2" fill="none"/>
                <circle cx="40" cy="51" r="3" fill="white"/>
                <line x1="40" y1="54" x2="40" y2="58" stroke="white" stroke-width="2" stroke-linecap="round"/>
              </svg>
              <p>Unidad de Auditoría<br>e Inspección</p>
            </div>

            <!-- Seguridad -->
            <div class="unidad-item">
              <svg class="unidad-shield green" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- casco de seguridad -->
                <path d="M18 50 Q18 34 40 34 Q62 34 62 50 Z" stroke="white" stroke-width="2" fill="white" fill-opacity=".15"/>
                <path d="M14 50 h52" stroke="white" stroke-width="2.5" stroke-linecap="round"/>
                <path d="M26 50 v4 Q26 58 40 58 Q54 58 54 54 v-4" stroke="white" stroke-width="1.5" fill="none"/>
                <path d="M30 34 Q40 24 50 34" stroke="white" stroke-width="2" fill="none" stroke-linecap="round"/>
              </svg>
              <p>Seguridad</p>
            </div>

            <!-- Higiene Industrial -->
            <div class="unidad-item">
              <svg class="unidad-shield teal" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- gráfica / industrial -->
                <rect x="20" y="28" width="40" height="28" rx="2" stroke="white" stroke-width="1.8" fill="none"/>
                <polyline points="26,50 33,38 40,44 49,32 54,40" stroke="white" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
                <line x1="20" y1="56" x2="60" y2="56" stroke="white" stroke-width="1" opacity=".5"/>
              </svg>
              <p>Higiene Industrial</p>
            </div>

            <!-- Laboratorio de Pruebas -->
            <div class="unidad-item">
              <svg class="unidad-shield green" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- microscopio -->
                <line x1="40" y1="26" x2="40" y2="44" stroke="white" stroke-width="2.5" stroke-linecap="round"/>
                <ellipse cx="40" cy="24" rx="7" ry="5" stroke="white" stroke-width="1.8" fill="none"/>
                <path d="M34 44 Q28 52 28 58 Q28 62 40 62 Q52 62 52 58 Q52 52 46 44 Z" stroke="white" stroke-width="1.8" fill="white" fill-opacity=".12"/>
                <line x1="28" y1="58" x2="52" y2="58" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
              </svg>
              <p>Laboratorio de<br>Pruebas</p>
            </div>

            <!-- Medio Ambiente -->
            <div class="unidad-item">
              <svg class="unidad-shield teal" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- hoja / medio ambiente -->
                <path d="M40 60 Q40 40 55 30 Q55 48 40 60 Z" fill="white" fill-opacity=".9"/>
                <path d="M40 60 Q40 40 25 30 Q25 48 40 60 Z" fill="white" fill-opacity=".5"/>
                <circle cx="40" cy="28" r="5" fill="white" fill-opacity=".7"/>
                <line x1="40" y1="60" x2="40" y2="68" stroke="white" stroke-width="2" stroke-linecap="round"/>
                <path d="M30 68 Q40 64 50 68" stroke="white" stroke-width="1.5" fill="none" stroke-linecap="round"/>
              </svg>
              <p>Medio Ambiente</p>
            </div>

            <!-- Ergonomía -->
            <div class="unidad-item">
              <svg class="unidad-shield green" viewBox="0 0 80 95" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="shield-bg" d="M6,0 L74,0 Q80,0 80,6 L80,62 L40,95 L0,62 L0,6 Q0,0 6,0 Z"/>
                <!-- silla ergonómica -->
                <rect x="28" y="38" width="24" height="14" rx="3" stroke="white" stroke-width="1.8" fill="none"/>
                <circle cx="40" cy="31" r="5" stroke="white" stroke-width="1.8" fill="none"/>
                <line x1="40" y1="52" x2="40" y2="62" stroke="white" stroke-width="2" stroke-linecap="round"/>
                <path d="M28 62 h24" stroke="white" stroke-width="2" stroke-linecap="round"/>
                <line x1="28" y1="38" x2="22" y2="50" stroke="white" stroke-width="1.8" stroke-linecap="round"/>
              </svg>
              <p>Ergonomía</p>
            </div>

          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- ===== QUIÉNES SOMOS (rediseño) ===== -->
  <section class="section socios-section reveal" id="mision">
    <div class="container">
      <div class="socios-grid">

        <!-- Columna izquierda: texto -->
        <div class="socios-text">
          <span class="socios-label">EN ARGA GROUP MÉXICO</span>
          <h2 class="socios-heading">
            Somos tus mejores socios estratégicos
            <strong>en el control total de pérdidas.</strong>
          </h2>
          <hr class="socios-divider">

          <div class="socios-features">
            <!-- Feature 1 -->
            <div class="socios-feat">
              <div class="socios-feat-icon">
                <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <circle cx="18" cy="16" r="7" fill="var(--lime)" opacity=".15" stroke="var(--lime)" stroke-width="2"/>
                  <circle cx="30" cy="16" r="7" fill="var(--lime)" opacity=".15" stroke="var(--lime)" stroke-width="2"/>
                  <ellipse cx="18" cy="36" rx="12" ry="7" fill="var(--lime)" opacity=".15" stroke="var(--lime)" stroke-width="2"/>
                  <ellipse cx="30" cy="36" rx="12" ry="7" fill="var(--lime)" opacity=".10" stroke="var(--lime)" stroke-width="2"/>
                </svg>
              </div>
              <div>
                <h4>Somos un grupo</h4>
                <p>Que brinda servicios integrales en seguridad, medio ambiente y administración de riesgos operacionales.</p>
              </div>
            </div>

            <!-- Feature 2 -->
            <div class="socios-feat">
              <div class="socios-feat-icon">
                <svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <rect x="18" y="4" width="12" height="8" rx="2" fill="var(--lime)" opacity=".2" stroke="var(--lime)" stroke-width="2"/>
                  <rect x="4" y="34" width="12" height="8" rx="2" fill="var(--lime)" opacity=".2" stroke="var(--lime)" stroke-width="2"/>
                  <rect x="18" y="34" width="12" height="8" rx="2" fill="var(--lime)" opacity=".2" stroke="var(--lime)" stroke-width="2"/>
                  <rect x="32" y="34" width="12" height="8" rx="2" fill="var(--lime)" opacity=".2" stroke="var(--lime)" stroke-width="2"/>
                  <line x1="24" y1="12" x2="24" y2="24" stroke="var(--lime)" stroke-width="2"/>
                  <line x1="10" y1="24" x2="38" y2="24" stroke="var(--lime)" stroke-width="2"/>
                  <line x1="10" y1="24" x2="10" y2="34" stroke="var(--lime)" stroke-width="2"/>
                  <line x1="24" y1="24" x2="24" y2="34" stroke="var(--lime)" stroke-width="2"/>
                  <line x1="38" y1="24" x2="38" y2="34" stroke="var(--lime)" stroke-width="2"/>
                </svg>
              </div>
              <div>
                <h4>Nuestras Divisiones</h4>
                <ul class="socios-divisions">
                  <li><strong>ARGA</strong> Consultoría</li>
                  <li><strong>ARGA</strong> Fire</li>
                  <li><strong>ARGA</strong> Training</li>
                  <li><strong>ARGA</strong> Labs</li>
                </ul>
              </div>
            </div>
          </div>
        </div>

        <!-- Columna derecha: imagen -->
        <div class="socios-img-wrap">
          <img src="/demo/img/Casa.webp" alt="ARGA Group instalaciones" class="socios-img">
        </div>

      </div>
    </div>
  </section>

  <!-- ===== POLÍTICAS ===== -->
  <section class="section policies-section reveal" id="politicas">
    <div class="mvv-gradient-bg">
      <div class="container">
        <div class="policies-grid">

          <!-- Política de Privacidad -->
          <div class="policy-card">
            <div class="policy-icon">
              <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
                <rect x="10" y="16" width="28" height="24" rx="3" stroke="var(--teal)" stroke-width="2"/>
                <path d="M16 16V12a8 8 0 1116 0v4" stroke="var(--teal)" stroke-width="2"/>
                <circle cx="24" cy="28" r="4" fill="var(--lime)" opacity="0.3" stroke="var(--lime)" stroke-width="2"/>
              </svg>
            </div>
            <div class="policy-divider"></div>
            <p class="policy-title">Política de Privacidad</p>
            <p class="policy-desc">Protección de datos personales conforme a la Ley Federal, aplicada a toda la información recopilada, procesada y resguardada por ARGA Group.</p>
            <a href="https://www.argamexico.com/site/avisoprivacidad" target="_blank" class="policy-link">Ver documento →</a>
          </div>

          <!-- Código de Ética -->
          <div class="policy-card">
            <div class="policy-icon">
              <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
                <rect x="8" y="4" width="28" height="36" rx="3" stroke="var(--teal)" stroke-width="2"/>
                <path d="M14 14h16M14 20h16M14 26h10" stroke="var(--teal)" stroke-width="2" stroke-linecap="round"/>
                <path d="M30 32l2 2 5-5" stroke="var(--lime)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </div>
            <div class="policy-divider"></div>
            <p class="policy-title">Código de Ética</p>
            <p class="policy-desc">Compromiso de actuar con integridad, honestidad y objetividad. Rechazo a prácticas de corrupción, dentro del alcance de nuestra acreditación EMA.</p>
            <a href="https://www.argamexico.com/site/codigoetica" target="_blank" class="policy-link">Ver documento →</a>
          </div>

          <!-- Procedimiento de Quejas -->
          <div class="policy-card">
            <div class="policy-icon">
              <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
                <path d="M8 8h32v24H26l-8 8v-8H8z" stroke="var(--teal)" stroke-width="2" stroke-linejoin="round"/>
                <path d="M16 18h16M16 24h10" stroke="var(--teal)" stroke-width="2" stroke-linecap="round"/>
              </svg>
            </div>
            <div class="policy-divider"></div>
            <p class="policy-title">Procedimiento de Quejas</p>
            <p class="policy-desc">Canal formal para atención de quejas bajo los valores de Honestidad, Calidad, Compromiso, Profesionalismo, Discreción y Lealtad.</p>
            <a href="https://www.argamexico.com/site/quejas" target="_blank" class="policy-link">Ver documento →</a>
          </div>

        </div>
      </div>
    </div>
  </section>


  <!-- ===== CLIENTES ===== -->
  <section class="section clients-section reveal" id="clientes">
    <div class="container">
      <h2 class="section-title">
        <span class="gradient-text">AGRADECEMOS LA CONFIANZA</span><br>
        <span class="gradient-text-alt">DE NUESTROS CLIENTES</span>
      </h2>
      <div class="clients-grid">
        <div class="client-logo">ADAMS</div>
        <div class="client-logo">ADIENT</div>
        <div class="client-logo">AMERICAN<br>STANDARD</div>
        <div class="client-logo">ANTOLIN</div>
        <div class="client-logo">AUDI</div>
        <div class="client-logo">AUTOTEK</div>
        <div class="client-logo">BMW</div>
        <div class="client-logo">CAMPRA</div>
        <div class="client-logo">CAPSUGEL</div>
        <div class="client-logo">FAURECIA</div>
        <div class="client-logo">FEDERAL<br>MOGUL</div>
        <div class="client-logo">GESTAMP</div>
        <div class="client-logo">IBERO</div>
        <div class="client-logo">IDESA</div>
        <div class="client-logo">KAYSER</div>
        <div class="client-logo">KIMBERLY<br>CLARK</div>
        <div class="client-logo">LONZA</div>
        <div class="client-logo">MONDELEZ</div>
        <div class="client-logo">SKF</div>
        <div class="client-logo">STANLEY</div>
        <div class="client-logo">THYSSENKRUPP</div>
        <div class="client-logo">UNILEVER</div>
        <div class="client-logo">UPAEP</div>
        <div class="client-logo">WHIRLPOOL</div>
      </div>
    </div>
  </section>

  <!-- ===== EQUIPO ===== -->
  <section class="section team-section reveal" id="equipo">
    <div class="team-inner">
      <div class="team-cards">
        <div class="team-card">
          <div class="team-avatar">
            <svg width="70" height="70" viewBox="0 0 70 70"><circle cx="35" cy="35" r="35" fill="#e8f4f8"/><circle cx="35" cy="26" r="12" fill="#6ebbd9"/><ellipse cx="35" cy="58" rx="20" ry="14" fill="#6ebbd9"/></svg>
          </div>
          <p class="team-name">ISH Alejandra M.</p>
          <p class="team-role-sub">Ing. en Seguridad e Higiene por el IPRL</p>
          <p class="team-role"><strong>Coordinadora de Seguridad</strong></p>
        </div>
        <div class="team-card featured">
          <div class="team-avatar">
            <svg width="70" height="70" viewBox="0 0 70 70"><circle cx="35" cy="35" r="35" fill="#eef8e8"/><circle cx="35" cy="26" r="12" fill="#93cc00"/><ellipse cx="35" cy="58" rx="20" ry="14" fill="#93cc00"/></svg>
          </div>
          <p class="team-name">ISH David G. Pér.</p>
          <p class="team-role-sub">Ing. en Seguridad e Higiene por el IPRL</p>
          <p class="team-role"><strong>Coordinador de Ergonomía</strong></p>
        </div>
        <div class="team-card">
          <div class="team-avatar">
            <svg width="70" height="70" viewBox="0 0 70 70"><circle cx="35" cy="35" r="35" fill="#e8f4f8"/><circle cx="35" cy="26" r="12" fill="#6ebbd9"/><ellipse cx="35" cy="58" rx="20" ry="14" fill="#6ebbd9"/></svg>
          </div>
          <p class="team-name">ISH Eduardo M.</p>
          <p class="team-role-sub">Ing. en Seguridad e Higiene por el IPRL</p>
          <p class="team-role"><strong>Técnico de Laboratorio</strong></p>
        </div>
      </div>

      <div class="team-info">
        <h2 class="gradient-text-outline">EQUIPO<br>DE<br>TRABAJO</h2>
        <p>Dicha experiencia ha quedado demostrada en empresas del giro de la construcción, petroquímica, alimentaría, industrial, comercial y de servicios. Donde el principal objetivo ha sido brindar un servicio de calidad, garantizando la integridad física de los colaboradores, la productividad, la continuidad del negocio, las afectaciones ambientales y la reducción de pérdidas por sanciones administrativas.</p>
        <a href="#contacto" class="btn-secondary">Contáctanos</a>
      </div>
    </div>
  </section>

  <!-- ===== ACREDITACIONES ===== -->
  <section class="acred-section reveal" id="acreditaciones">
    <div class="container">
      <p class="acred-label">Organismos que nos avalan</p>
      <h2 class="acred-title">Acreditaciones y Aprobaciones</h2>
      <div class="acred-grid">

        <!-- EMA — Higiene Industrial -->
        <div class="acred-card">
          <div class="acred-logo-wrap">
            <img src="/demo/img/emma.png" alt="EMA" class="acred-logo">
          </div>
          <div class="acred-body">
            <span class="acred-org">Entidad Mexicana de Acreditación</span>
            <span class="acred-code">AL-1556-140/22</span>
            <span class="acred-desc">Laboratorio de Higiene Industrial</span>
          </div>
        </div>

        <!-- EMA — Fuentes Fijas -->
        <div class="acred-card">
          <div class="acred-logo-wrap">
            <img src="/demo/img/emma.png" alt="EMA" class="acred-logo">
          </div>
          <div class="acred-body">
            <span class="acred-org">Entidad Mexicana de Acreditación</span>
            <span class="acred-code">FF-1555-128/22</span>
            <span class="acred-desc">Laboratorio de Fuentes Fijas</span>
          </div>
        </div>

        <!-- STPS -->
        <div class="acred-card">
          <div class="acred-logo-wrap">
            <img src="/demo/img/stps.png" alt="STPS" class="acred-logo">
          </div>
          <div class="acred-body">
            <span class="acred-org">Secretaría del Trabajo y Previsión Social</span>
            <span class="acred-code">LPSTPS-169/2025</span>
            <span class="acred-desc">Laboratorio de Pruebas aprobado</span>
          </div>
        </div>

        <!-- PROFEPA -->
        <div class="acred-card">
          <div class="acred-logo-wrap">
            <img src="/demo/img/profepa.png" alt="PROFEPA" class="acred-logo">
          </div>
          <div class="acred-body">
            <span class="acred-org">Procuraduría Federal de Protección al Ambiente</span>
            <span class="acred-code">PFPA-APR-LP-RUIDO-016/2023</span>
            <span class="acred-desc">Laboratorio de Ruido Ambiental</span>
          </div>
        </div>

      </div>
    </div>
  </section>

  <!-- ===== CONTACTO ===== -->
  <section class="section contact-section reveal" id="contacto">
    <div class="container">
      <h2 class="section-title contact-slogan">
        <span class="gradient-text">CERO ACCIDENTES NO ES UNA META.</span><br>
        <span class="gradient-text-alt">ES NUESTRO ESTÁNDAR.</span>
      </h2>

      <div class="contact-grid">
        <div class="contact-info">
          <h3>Contáctanos</h3>
          <ul>
            <li>
              <span class="contact-icon">📍</span>
              <span>Calle 97, Bosques de Amalucan<br>Puebla, Pue. México CP 72310</span>
            </li>
            <li>
              <span class="contact-icon">✉️</span>
              <span>ventas@argamexico.com</span>
            </li>
            <li>
              <span class="contact-icon">✉️</span>
              <span>ventas@argafire.com</span>
            </li>
            <li>
              <span class="contact-icon">📞</span>
              <span>(221) 859 2152</span>
            </li>
            <li>
              <span class="contact-icon">🌐</span>
              <span>argamexico.com</span>
            </li>
          </ul>
        </div>

        <div class="contact-map">
          <iframe
            src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d939.4!2d-98.132538!3d19.050812!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zMTnCsDAzJzAyLjkiTiA5OMKwMDcnNTcuMSJX!5e0!3m2!1ses!2smx!4v1"
            width="100%"
            height="100%"
            style="border:0; border-radius:16px;"
            allowfullscreen=""
            loading="lazy"
            referrerpolicy="no-referrer-when-downgrade"
            title="Ubicación ARGA Group México">
          </iframe>
        </div>
      </div>
    </div>
  </section>

  <!-- ===== FOOTER ===== -->
  <footer class="site-footer">
    <!-- Acento superior -->
    <div class="footer-accent"></div>

    <div class="container">
      <div class="footer-grid">

        <!-- Columna 1: Marca -->
        <div class="footer-col footer-col--brand">
          <img src="/demo/img/argaBlanco.png" alt="ARGA Group México" class="footer-logo">
          <p class="footer-tagline">Más de 30 años protegiendo empresas.<br>Eficientes, Seguras y Rentables.</p>
          <!-- Redes sociales -->
          <div class="footer-social">
            <a href="https://www.linkedin.com/company/arga-group-mexico" target="_blank" aria-label="LinkedIn" class="footer-social-link">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
            </a>
            <a href="https://www.facebook.com/ArgaGroupMexico" target="_blank" aria-label="Facebook" class="footer-social-link">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
            </a>
            <a href="https://www.instagram.com/argagroupmexico" target="_blank" aria-label="Instagram" class="footer-social-link">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
            </a>
          </div>
        </div>

        <!-- Columna 2: Navegación -->
        <div class="footer-col">
          <h4 class="footer-col-title">Navegación</h4>
          <nav class="footer-links">
            <a href="#inicio">Inicio</a>
            <a href="#quienes">Quiénes Somos</a>
            <a href="#mision">Socios Estratégicos</a>
            <a href="#politicas">Políticas</a>
            <a href="#clientes">Clientes</a>
            <a href="#acreditaciones">Acreditaciones</a>
            <a href="#contacto">Contacto</a>
          </nav>
        </div>

      </div><!-- /footer-grid -->

      <!-- Línea divisora + copyright -->
      <div class="footer-bottom">
        <hr class="footer-divider">
        <p class="footer-copy">© 2025 Empresas y Negocios GrupoEHS S.A. de C.V. Todos los derechos reservados.</p>
      </div>
    </div>
  </footer>

  <!-- ===== MASCOTA TIGRE — aparece desde bordes aleatorios ===== -->
  <div id="tigreOverlay" aria-hidden="true">
    <!-- Contenedor del tigre + burbuja (se posiciona por JS) -->
    <div id="tigreUnit">
      <div id="tigreBubble"><p id="tigreBubbleText"></p></div>
      <img id="tigreImg" src="/demo/img/tigresitobb.png" alt="Mascota ARGA Group">
    </div>
  </div>

  <!-- Filtros SVG para anillos en escudos de unidades -->
  <svg xmlns="http://www.w3.org/2000/svg" width="0" height="0" style="position:absolute;overflow:hidden">
    <defs>
      <!-- Anillo azul (shields teal) -->
      <filter id="ringTeal" x="-25%" y="-25%" width="150%" height="150%">
        <feMorphology in="SourceAlpha" operator="dilate" radius="9" result="outer"/>
        <feFlood flood-color="#2E6AC9" flood-opacity="0.18" result="outerColor"/>
        <feComposite in="outerColor" in2="outer" operator="in" result="outerRing"/>
        <feMorphology in="SourceAlpha" operator="dilate" radius="4" result="inner"/>
        <feFlood flood-color="rgba(255,255,255,0.55)" result="innerColor"/>
        <feComposite in="innerColor" in2="inner" operator="in" result="innerRing"/>
        <feMerge>
          <feMergeNode in="outerRing"/>
          <feMergeNode in="innerRing"/>
          <feMergeNode in="SourceGraphic"/>
        </feMerge>
      </filter>
      <!-- Anillo rojo (shields green/red) -->
      <filter id="ringRed" x="-25%" y="-25%" width="150%" height="150%">
        <feMorphology in="SourceAlpha" operator="dilate" radius="9" result="outer"/>
        <feFlood flood-color="#C41230" flood-opacity="0.18" result="outerColor"/>
        <feComposite in="outerColor" in2="outer" operator="in" result="outerRing"/>
        <feMorphology in="SourceAlpha" operator="dilate" radius="4" result="inner"/>
        <feFlood flood-color="rgba(255,255,255,0.55)" result="innerColor"/>
        <feComposite in="innerColor" in2="inner" operator="in" result="innerRing"/>
        <feMerge>
          <feMergeNode in="outerRing"/>
          <feMergeNode in="innerRing"/>
          <feMergeNode in="SourceGraphic"/>
        </feMerge>
      </filter>
    </defs>
  </svg>

  <script src="/demo/script.js"></script>
</body>

</html>
